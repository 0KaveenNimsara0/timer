...
export function useTimer() {
  ...
  const saveToHistory = useCallback(async (durationOverride?: number) => {
    ...
    setHistory(prev => {
      ...
      AsyncStorage.setItem('timer_history', JSON.stringify(updated)).catch(e => console.error(e));
      return updated;
    });
  }, [mode, time]);
  ...
  const playBeep = () => {
    // React Native Vibration API instead of AudioContext
    Vibration.vibrate([0, 500, 200, 500]); // Wait 0ms, vibrate 500ms, pause 200ms, vibrate 500ms
  };
